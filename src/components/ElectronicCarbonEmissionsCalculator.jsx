...
import "./ElectronicCarbonEmissionsCalculator.css"; // Import the CSS file for styling

const ElectronicCarbonEmissionsCalculator = () => {
  const [device, setDevice] = useState("50"); // Default device wattage
  const [hours, setHours] = useState(1); // Default usage hours
  const [carbonIntensity, setCarbonIntensity] = useState(500); // Default carbon intensity
  const [results, setResults] = useState([]); // Store calculation results
